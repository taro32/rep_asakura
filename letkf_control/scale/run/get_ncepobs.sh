...
while ((time <= ETIME)); do
  ...
  for hh in '00' '06' '12' '18'; do
    ...
    if (("$yyyy$mm" >= 200908)); then
      mv -f $TMPS/download/${yyyy}${mm}${dd}.nr/prepbufr.gdas.$yyyy$mm$dd.t${hh}z.nr \
            $TMPS/download/prepbufr.gdas.${timef}.nr
    elif (("$yyyy$mm" >= 200807)); then
      mv -f $TMPS/download/prepbufr.gdas.$yyyy$mm$dd.t${hh}z.nr \
            $TMPS/download/prepbufr.gdas.${timef}.nr
    else
    ...
    mv $TMPS/download/prepbufr.gdas.${timef}.nr $TMPS/prepbufr.in
