...
workdir_letkf2 = '/work/gv42/f00019/enkc_with_TC/20250916_tchires_letkc_L1_negativeQonly_lamda099_psobs_target960error1_window1h/result/tc_hires/200001'
...
for day in range(4,10):
    ...
    for hour in range(0,endhour,3): #3
        ...
        #minpres_letkf[i] = pres[0,0,200,200]/100
        pres = data.variables['MSLP']
        minpres_letkf[i] = np.min(pres[0,:,:],axis=(0,1))/100
        #minpres_letkf[i] = pres[0,200,200]/100
        if day < 7:
        ...
        #minpres_mdet[i] = pres[0,0,200,200]/100
        pres = data.variables['MSLP']
        minpres_mdet[i] = np.min(pres[0,:,:],axis=(0,1))/100
        #minpres_mdet[i] = pres[0,200,200]/100
        i = i + 1

print(minpres_letkf)
print(minpres_mdet)
plt.plot(minpres_letkf[:],color='black')
plt.plot(minpres_mdet[:],color='green')
plt.ylim(978,995)
#plt.ylim(950,970)
plt.xlim(0,47)
#plt.savefig('TCpres_nocntlvscntl_local095.png')
plt.savefig('pres_tchires_letkc_L1_negativeQonly_lamda099_psobs_target960error1_window1h.png')
plt.show()
