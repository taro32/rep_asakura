...
hour = 0
i = 0
rain_letkf = np.zeros((72))
rain_mdet = np.zeros((72))
#print(minpres_letkf)
for day in range(4,10):
    ...
    for hour in range(0,endhour,3): #3
        ...
        #print(type(u10m))
        rain_letkf[i]=np.average(rain[0,:,:],axis=(0,1))*3600
        if day < 7:
        ...
        rain = data.variables['RAIN'][:,:,:]
        #print(type(u10m))
        rain_mdet[i]=np.average(rain[0,:,:],axis=(0,1))*3600
        i = i + 1

print(rain_letkf)
print(rain_mdet)
plt.plot(rain_letkf[:],color='black')
plt.plot(rain_mdet[:],color='green')
#plt.ylim(978,995)
#plt.ylim(950,970)
plt.xlim(0,47)
#plt.savefig('TCpres_nocntlvscntl_local095.png')
plt.savefig('rain_tchires_letkc_L1_negativeQonly_lamda09_psobs_target960error1_window1h.png')
plt.show()
