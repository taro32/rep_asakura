...
if [ "$TOPO_FORMAT" = "GTOPO30" ] || [ "$TOPO_FORMAT" = "DEM50M" ] || [ "$LANDUSE_FORMAT" = "GLCCv2" ] || [ "$LANDUSE_FORMAT" = "LU100M" ] ; then
  ...
  #config_file_scale_launcher fcst fcst_scale-rm_pp_ens "f$fmember/pp" 1
...
while ((time_s <= ETIME)); do
  ...
  if ((BDY_FORMAT != 0)); then
    ...
    #config_file_scale_launcher fcst fcst_scale-rm_init_ens "f$fmember/init" $((m_run_onecycle*rcycle))
...
#        fi
  ...
  #config_file_scale_launcher fcst fcst_scale-rm_ens "f$fmember/run" $((fmember*rcycle))
